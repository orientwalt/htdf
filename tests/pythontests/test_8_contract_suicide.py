...
ABI, BYTECODES = parse_truffe_compile_outputs('./sol/Suicide.json')
#
#
...
def test_deploy_contract(conftest_args):
    ...
    gas_wanted = 5000000
    gas_price = 100
    tx_amount = 100*10**8 # send initial amount while constructing contract
    data = BYTECODES
...
def test_kill(conftest_args):
    ...
    killTx = hc.functions.kill(
    ).buildTransaction_htdf()

    data = remove_0x_prefix(killTx['data'])
    print('========> data{}'.format(remove_0x_prefix(killTx['data'])))
    signed_tx = HtdfTxBuilder(
        from_address=from_addr,
    ...
    # due to selfdestruct will refund some coins.
    suicide_refund_satoshi = 100*10**8 + (tx_amount - tx_amount)
    assert  from_acc_new.balance_satoshi == from_acc.balance_satoshi - gas_price * int(tx['gas_used']) + suicide_refund_satoshi
