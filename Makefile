...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
SDK_PACK := $(shell go list -m github.com/orientwalt/htdf | sed  's/ /\@/g')
...
ldflags = -X github.com/orientwalt/htdf/version.Name=iris \
		  -X github.com/orientwalt/htdf/version.AppName=iris \
		  -X github.com/orientwalt/htdf/version.Version=$(VERSION) \
		  -X github.com/orientwalt/htdf/version.Commit=$(COMMIT) \
		  -X "github.com/orientwalt/htdf/version.BuildTags=$(build_tags_comma_sep)" \
		  -X github.com/orientwalt/htdf/types.reDnmString=[a-zA-Z][a-zA-Z0-9/:]{2,127}

ifeq ($(WITH_CLEVELDB),yes)
  ldflags += -X github.com/orientwalt/htdf/types.DBBackend=cleveldb
endif
ldflags += $(LDFLAGS)
