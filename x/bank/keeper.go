...
func (keeper BaseKeeper) IncreaseLoosenToken(
	...
	keeper.ak.IncreaseTotalLoosenToken(ctx, amt)
}

// SendCoins moves coins from one account to another
func (keeper BaseKeeper) DecreaseLoosenToken(
	ctx sdk.Context, amt sdk.Coins) {
	keeper.ak.DecreaseTotalLoosenToken(ctx, amt)
}
...
func (keeper BaseKeeper) BurnCoinsFromAddr(
	...
	_, _, err := subtractCoins(ctx, keeper.ak, fromAddr, amt)
	if err != nil {
		return nil, err
	}
	return burnCoins(ctx, keeper.ak, fromAddr.String(), amt)
}
...
func (keeper BaseKeeper) BurnCoinsFromPool(
	...
	return burnCoins(ctx, keeper.ak, pool, amt)
}
