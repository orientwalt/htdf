...
func TestAnteHandlerFees(t *testing.T) {
	...
	require.True(t, input.fck.GetCollectedFees(ctx).IsEqual(sdk.NewCoins()))
...
func TestAnteHandlerBadSignBytes(t *testing.T) {
	...
	for _, cs := range cases {
		tx := newTestTxWithSignBytes(
			...
			xauth.StdSignBytes(cs.chainID, cs.accnum, cs.seq, cs.fee, cs.msgs, ""),
			"",
		)
...
func TestAnteHandlerSetPubKey(t *testing.T) {
	...
	msgs = []sdk.Msg{msg}
	tx = newTestTx(ctx, msgs, privs, []uint64{1}, seqs, fee)
	sigs := tx.(xauth.StdTx).GetSignatures()
	sigs[0].PubKey = nil
	checkInvalidTx(t, anteHandler, ctx, tx, false, sdk.CodeInvalidPubKey)
...
func TestProcessPubKey(t *testing.T) {
	...
	tests := []struct {
		...
		{"no sigs, simulate off", args{acc1, xauth.StdSignature{}, false}, true},
		{"no sigs, simulate on", args{acc1, xauth.StdSignature{}, true}, false},
		{"no sigs, account with pub, simulate on", args{acc2, xauth.StdSignature{}, true}, false},
		{"pubkey doesn't match addr, simulate off", args{acc1, xauth.StdSignature{PubKey: priv2.PubKey()}, false}, true},
		{"pubkey doesn't match addr, simulate on", args{acc1, xauth.StdSignature{PubKey: priv2.PubKey()}, true}, false},
	}
...
func TestConsumeSignatureVerificationGas(t *testing.T) {
	...
	tests := []struct {
		...
		{"PubKeyEd25519", args{sdk.NewInfiniteGasMeter(), nil, ed25519.GenPrivKey().PubKey(), params}, xauth.DefaultSigVerifyCostED25519, true},
		{"PubKeySecp256k1", args{sdk.NewInfiniteGasMeter(), nil, secp256k1.GenPrivKey().PubKey(), params}, xauth.DefaultSigVerifyCostSecp256k1, false},
...
func expectedGasCostByKeys(pubkeys []crypto.PubKey) uint64 {
	...
	for _, pubkey := range pubkeys {
		...
		case strings.Contains(pubkeyType, "ed25519"):
			cost += xauth.DefaultParams().SigVerifyCostED25519
		case strings.Contains(pubkeyType, "secp256k1"):
			cost += xauth.DefaultParams().SigVerifyCostSecp256k1
		default:
...
func TestEnsureSufficientMempoolFees(t *testing.T) {
	...
	testCases := []struct {
		...
		{xauth.NewStdFee(200000, 5), false},
		{xauth.NewStdFee(200000, 1), false},
		{xauth.NewStdFee(200000, 20), true},
		{xauth.NewStdFee(200000, 30), true},
		{xauth.NewStdFee(200000, 40), true},
		{xauth.NewStdFee(200000, 50), true},
	}
